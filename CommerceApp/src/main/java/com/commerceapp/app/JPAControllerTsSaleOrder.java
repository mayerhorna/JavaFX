...
public class JPAControllerTsSaleOrder {
	...
	public Integer obtenerIdOrdenVentaPorCodigo(String codigo) {
	    ...
	    try {
	        TypedQuery<Integer> query = entityManager.createQuery("SELECT o.id FROM TsSaleOrder o WHERE o.code = :codigo", Integer.class);
	...
	public String crearOrdenVenta(TsSaleOrder ordenVenta) {
		...
		try {
			...
			Query query = entityManager
					.createNativeQuery("SELECT seq_count FROM se_tb_ts_sale_order WHERE seq_name = 'ts_sale_order_seq'");
