...
public class JPAControllerTsSaleOrder {
	...
	public String obtenerCodigoOrdenVentaPorId(Integer id) {
		String codigoOrdenVenta = null;
		try {
			TypedQuery<String> query = entityManager.createQuery("SELECT o.code FROM TsSaleOrder o WHERE o.id = :id",
					String.class);
			query.setParameter("id", id);
			codigoOrdenVenta = query.getSingleResult();
		} catch (NoResultException e) {
			// Si no se encuentra ninguna orden de venta con ese ID, devolvemos null
			System.out.println("No se encontró ninguna orden de venta con el ID: " + id);
		} catch (Exception e) {
			// Si ocurre alguna otra excepción, imprimimos la pila de errores
			e.printStackTrace();
		}
	...
	public Object[] obtenerInfoPedidoPorId(int idpedido) {
		StoredProcedureQuery query = entityManager.createNamedStoredProcedureQuery("obtenerinfopedido");
